<?php

declare(strict_types=1);

namespace App\Modules\Messaging\Infrastructure\Mail;

use App\Infrastructure\Mail\MailerInterface;
use App\Infrastructure\Templating\RendererInterface;
use App\Modules\Messaging\Domain\Exception\ContactMessageNotificationException;
use App\Modules\Messaging\Domain\Service\ContactMessageNotifierInterface;
use Throwable;

use function array_filter;
use function array_map;
use function array_unique;
use function count;
use function date;
use function filter_var;
use function implode;
use function is_array;
use function preg_split;
use function trim;

use const FILTER_VALIDATE_EMAIL;
use const PREG_SPLIT_NO_EMPTY;

final readonly class ContactMessageMailer implements ContactMessageNotifierInterface
{
    public function __construct(
        private MailerInterface $mailer,
        private RendererInterface $renderer,
        private string $adminNotifications,
        private string $templateBasePath,
    ) {}

    public function notifyAdminOfNewContact(
        int $threadId,
        string $name,
        string $email,
        ?string $phone,
        string $subject,
        string $message,
    ): void {
        $recipients = $this->extractRecipients($this->adminNotifications);

        if ($recipients === []) {
            throw ContactMessageNotificationException::missingRecipients();
        }

        $subjectLine = "Nuevo mensaje de contacto #{$threadId}: {$subject}";
        $templatePath = $this->templateBasePath . "/templates/emails/contact-notification.latte";

        $body = $this->renderer->renderToString(
            $templatePath,
            [
                "threadId" => $threadId,
                "name" => $name,
                "email" => $email,
                "phone" => $phone,
                "subject" => $subject,
                "message" => $message,
                "year" => date("Y"),
            ],
        );

        $altBody = $this->buildAltBody(
            threadId: $threadId,
            name: $name,
            email: $email,
            phone: $phone,
            subject: $subject,
            message: $message,
        );

        try {
            $this->mailer->send($recipients, $subjectLine, $body, $altBody);
        } catch (Throwable $exception) {
            throw new ContactMessageNotificationException(
                "No fue posible notificar al equipo administrador.",
                previous: $exception,
            );
        }
    }

    /**
     * @return string[]
     */
    private function extractRecipients(string $rawRecipients): array
    {
        $pieces = preg_split('/[\s,;]+/', trim($rawRecipients), -1, PREG_SPLIT_NO_EMPTY);

        if (!is_array($pieces) || count($pieces) === 0) {
            return [];
        }

        $normalized = array_map(static fn(string $value): string => trim($value), $pieces);

        $validEmails = array_filter(
            $normalized,
            static fn(string $value): bool => (bool) filter_var($value, FILTER_VALIDATE_EMAIL),
        );

        return array_values(array_unique($validEmails));
    }

    private function buildAltBody(
        int $threadId,
        string $name,
        string $email,
        ?string $phone,
        string $subject,
        string $message,
    ): string {
        $phoneText = $phone !== null && trim($phone) !== "" ? $phone : "No proporcionado";

        return implode("\n", [
            "Nuevo mensaje de contacto",
            "Hilo: #{$threadId}",
            "Nombre: {$name}",
            "Correo: {$email}",
            "Telefono: {$phoneText}",
            "Asunto: {$subject}",
            "Mensaje:",
            $message,
        ]);
    }
}
