    ...
    public function update(Loan $loan): void
    {
        $stmt = $this->pdo->prepare("
            UPDATE loans SET
                ...
                signature_validation_date = :signature_validation_date,
        ...
        $stmt->execute([
            ...
            'first_payment_date' => $loan->firstPaymentDate()?->format('Y-m-d'),
            'last_scheduled_payment_date' => $loan->lastScheduledPaymentDate()?->format('Y-m-d'),
            'document_review_date' => $loan->documentReviewDate()?->format('Y-m-d H:i:s'),
            'approval_date' => $loan->approvalDate()?->format('Y-m-d H:i:s'),
            'document_generation_date' => $loan->documentGenerationDate()?->format('Y-m-d H:i:s'),
            'signature_validation_date' => $loan->signatureValidationDate()?->format('Y-m-d H:i:s'),
            ...
            'admin_observations' => $loan->adminObservations(),
            'internal_observations' => $loan->internalObservations(),
            'finance_signatory' => $loan->financeSignatory(),
            'lender_signatory' => $loan->lenderSignatory(),
    ...
    public function findDetailById(int $loanId): ?array
    {
        $stmt = $this->pdo->prepare("
            SELECT
                ...
                u.bank_name                        AS borrower_bank_name,
                u.interbank_code                   AS borrower_interbank_code,
                u.bank_account                     AS borrower_bank_account,
