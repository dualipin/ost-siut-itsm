...
if ($detailId > 0) {
    ...
    if ($publication === null || $publication->type !== PublicationTypeEnum::Format) {
        $redirector->to("/portal/publicaciones/formatos.php", [
            "error" => "El formato solicitado no existe.",
        ])->send();
    }
    ...
    $renderer->render(__DIR__ . "/avisos.show.latte", [
        ...
        "backUrl" => "/portal/publicaciones/formatos.php",
...
$formatos = array_map(
    ...
    $findByType->execute(PublicationTypeEnum::Format),
);

$formatosPorAnio = [];

foreach ($formatos as $formato) {
    $anio = $formato["fecha_publicacion"]?->format("Y") ?? "Sin fecha";

    if (!isset($formatosPorAnio[$anio])) {
        $formatosPorAnio[$anio] = [];
    }

    $formatosPorAnio[$anio][] = $formato;
}
...
$renderer->render(__DIR__ . "/formatos.latte", [
    "formatosPorAnio" => $formatosPorAnio,
    "error" => $request->input("error"),
    "success" => $request->input("success"),
]);
