...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $adjuntoActual = null;
    $stmtAdjunto = $conn->prepare('SELECT adjunto FROM documentos_tramites_ante_autoridades WHERE id = ?');
    ...
    $sql = "UPDATE `documentos_tramites_ante_autoridades` SET `titulo`= ?,`contenido`= ?,`adjunto`= ?,`fecha_documento`= ?,`privado`= ? WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        if ($adjunto !== $adjuntoActual && $adjunto) {
            $rutaArchivo = __DIR__ . '/../../../privado/archivos/repositorios/tramite-ante-autoridades/' . $adjunto;
