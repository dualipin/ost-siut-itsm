...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->execute()) {
        $mensaje = "Acervo bibliográfico registrado con éxito.";
    } else {
        if ($nombreArchivo) {
            $rutaArchivo = __DIR__ . '/../../archivos/subidos/acervos_bibliograficos/' . $nombreArchivo;
            if (file_exists($rutaArchivo)) {
                @unlink($rutaArchivo);
            }
        }
        $error = "Error al registrar el acervo bibliográfico.";
    }
