        ...
        if (!$descuentosJson) {
            ...
            if (!is_array($descuentos) || empty($descuentos)) {
                ...
                foreach ($descuentos as $index => $descuento) {
                    ...
                    $categoria = array_filter(
                        $categoriasTipoIngreso,
                        fn($c) => $c->id === $tipoId,
                    );
                    ...
                    try {
                        ...
                        $error =
                            "Error al simular {$categoria->nombre}: " .
                            $e->getMessage();
                        break;
                    }
                }
...
$simulacionesFormateadas = array_map(
    function ($sim, $index) {
        $corrida_con_numero = array_map(
            function ($pago, $num) {
                return [
                    "numero" => $num + 1,
                    "dto" => $pago,
