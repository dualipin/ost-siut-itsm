...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        switch ($action) {
            case 'aprobar': {
                $approvedAmount   = filter_var($_POST['approved_amount'] ?? '', FILTER_VALIDATE_FLOAT);
                $interestRate     = filter_var($_POST['applied_interest_rate'] ?? '', FILTER_VALIDATE_FLOAT);
                $dailyDefaultRate = filter_var($_POST['daily_default_rate'] ?? '', FILTER_VALIDATE_FLOAT);
                $termFortnights   = filter_var($_POST['term_fortnights'] ?? '', FILTER_VALIDATE_INT);
                $adminObs         = trim((string) ($_POST['admin_observations'] ?? '')) ?: null;
                ...
                $reviewerStmt = $db->prepare('SELECT curp FROM users WHERE user_id = :id');
            ...
            case 'rechazar': {
                ...
                $reviewUseCase->reject($loanId, $currentUser->id, $rejectionReason);
        ...
        $errors[] = 'El préstamo no puede ser procesado en su estado actual: ' . $e->getMessage();
    } catch (\Throwable $e) {
