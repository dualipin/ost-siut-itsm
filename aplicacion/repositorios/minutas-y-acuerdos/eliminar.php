...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['id_doc'])) {
        ...
        try {
            ...
            $adjuntoStmt = $conn->prepare('SELECT adjunto FROM documentos_minutas WHERE id = ?');
            $adjuntoStmt->execute([$documentoId]);
            $resultado = $adjuntoStmt->fetch(PDO::FETCH_ASSOC);

            if ($resultado && !empty($resultado['adjunto'])) {
                $rutaArchivo = realpath(__DIR__ . '/../../../privado/archivos/repositorios/minutas-y-acuerdos/' . $resultado['adjunto']);
                if ($rutaArchivo && file_exists($rutaArchivo)) {
                    unlink($rutaArchivo);
                }
            }

            $stmt = $conn->prepare('DELETE FROM documentos_minutas WHERE id = ?');
