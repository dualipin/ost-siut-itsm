...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $adjuntoActual = null;
    $stmtAdjunto = $conn->prepare('SELECT adjunto FROM documentos_gremiales WHERE id = ?');
    ...
    if (isset($_FILES['archivo']) && $_FILES['archivo']['error'] === UPLOAD_ERR_OK) {
        $directorio = __DIR__ . '/../../../privado/archivos/repositorios/documentos-gremiales/';
    ...
    $sql = "UPDATE `documentos_gremiales` SET `titulo`= ?,`contenido`= ?,`adjunto`= ?,`fecha_documento`= ?,`privado`= ? WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        if ($adjunto !== $adjuntoActual && $adjunto) {
            $rutaArchivo = __DIR__ . '/../../../privado/archivos/repositorios/documentos-gremiales/' . $adjunto;
